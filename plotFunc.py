...
def plotBandWidth(sizes, bandWidth):
    cpuFile = open('cacheDetails.dat', 'r')
    lineList = cpuFile.lineList()
    for itr in range(3, len(lineList)):
        temp = lineList[itr].split()[2]
        if temp[2].lower == 'kb':
    ...
    cacheLabelKeyList = cacheLabel.keys()

    plt.figure(1)
    plt.loglog(sizes, bandWidth, marker='o')
    plt.loglog(cacheKeyList[0], cpu_y, label=cacheLabelKeyList[0] + ':' +
               cacheLabel[cacheLabelKeyList[0]])
    plt.loglog(cacheKeyList[1], cpu_y, label=cacheLabelKeyList[1] + ':' +
               cacheLabel[cacheLabelKeyList[1]])
    plt.loglog(cacheKeyList[2], cpu_y, label=cacheLabelKeyList[2] + ':' +
               cacheLabel[cacheLabelKeyList[2]])
    plt.loglog(cacheKeyList[3], cpu_y, label=cacheLabelKeyList[3] + ':' +
